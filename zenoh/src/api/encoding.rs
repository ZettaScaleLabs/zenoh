...
impl Encoding {
    ...
    ///
    /// This encoding supposes that the payload was created with `ZBytes::from::<Vec<u8>>` or similar and
    /// it's data should be accessed with `ZBytes::to_bytes()`, no additional assumptions is made
    ...
    ///
    /// This encoding supposes that the payload was created with `ZBytes::from::<String>` or similar (`&str`, `Cow<str>`, `char`) and
    /// it's data can be acquired with `ZBytes::try_to_string()` withour an error.
